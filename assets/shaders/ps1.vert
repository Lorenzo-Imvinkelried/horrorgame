...
uniform bool u_TorchActive;
uniform vec3 u_TorchPos;
uniform int u_NumWorldTorches;
uniform vec4 u_WorldTorches[8]; // xyz: pos, w: intensity
...
void main()
{
    ...
    if (u_IsInstanced) {
        ...
        // Non-instanced objects: transform to world space
        worldPos = vec3(u_Model * vec4(aPos, 1.0));
        normalVec = mat3(u_Model) * aNormal;
    }
    ...
    if (u_WindStrength > 0.0) {
        ...
        if (windEnabled > 0.0) {
            ...
            float hFactor = normalizedH * normalizedH;
    ...
    if (u_ParticleMode) {
        ...
        vec3 fireColor = vec3(1.0, 0.74, 0.38);
        ...
        if (u_TorchActive) {
            ...
            if (dist < 24.0) {
                ...
                att = att * att;
                float torchDiff = max(dot(normalVec, lightVec), 0.0);
        ...
        for (int i = 0; i < 8; ++i) {
            ...
            if (dist < 20.0) {
                ...
                att = att * att;
                float tDiff = max(dot(normalVec, lightVec), 0.0);
