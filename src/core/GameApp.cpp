...
GameApp::GameApp()
    : m_chunkManager(Config::World::RenderDistance)
{
    s_instance = this;
}
...
bool GameApp::Init() {
    ...
    glfwSetWindowFocusCallback(m_window, [](GLFWwindow*, int focused) {
        if (focused == GLFW_FALSE && GameApp::GetInstance()) {
    ...
    // Initialize Graphic Pipelines & Subsystems
...
void GameApp::updateGameLogic(float deltaTime) {
    ...
    m_projectiles.Update(deltaTime, &m_player, m_particles, m_damageNumbers,
...
void GameApp::UpdateFrame() {
    ...
    while (m_window.pollEvent(event)) {
        ...
        if (event.type == sf::Event::LostFocus) m_inputManager.SetGamePaused(true);
    ...
    if (m_inputManager.IsShovelMode() && m_hasTerraTarget && (isDigging || isBuilding)) {
        ...
        m_chunkManager.ModifyTerrain(m_terraTarget.x, m_terraTarget.z, 3.6f, deltaH, &m_particles);
    }
