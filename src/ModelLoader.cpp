...
void ModelLoader::GenerateMesh(const std::vector<BoxDef>& boxes, std::vector<float>& outVertices) {
    ...
    for (const auto& box : boxes) {
        ...
        for (int i = 0; i < 36; i++) {
            glm::vec4 p(s_cubeVertices[i*8 + 0], s_cubeVertices[i*8 + 1], s_cubeVertices[i*8 + 2], 1.0f);
            glm::vec4 n(s_cubeVertices[i*8 + 3], s_cubeVertices[i*8 + 4], s_cubeVertices[i*8 + 5], 0.0f);
            float u = s_cubeVertices[i*8 + 6];
            float v = s_cubeVertices[i*8 + 7];
...
void ModelLoader::GenerateMeshTransformed(const std::vector<TransformedBox>& boxes, std::vector<float>& outVertices) {
    for (const auto& box : boxes) {
        for (int i = 0; i < 36; i++) {
            glm::vec4 p(s_cubeVertices[i*8 + 0], s_cubeVertices[i*8 + 1], s_cubeVertices[i*8 + 2], 1.0f);
            glm::vec4 n(s_cubeVertices[i*8 + 3], s_cubeVertices[i*8 + 4], s_cubeVertices[i*8 + 5], 0.0f);
            ...
            glm::vec4 transP = box.Transform * p;
            glm::vec4 transN = box.Transform * n;
